import type { IAddConfettiConfig, INormalizedAddConfettiConfig } from './types';
import {
...
function normalizeConfettiConfig(confettiConfig: IAddConfettiConfig): INormalizedAddConfettiConfig {
  const {
    ...
    confettiNumber = confettiConfig.confettiesNumber ||
      (confettiConfig.emojis ? DEFAULT_EMOJIS_NUMBER : DEFAULT_CONFETTI_NUMBER),
    confettiColors = DEFAULT_CONFETTI_COLORS,
    emojiColors = DEFAULT_CONFETTI_COLORS,
  ...
  return { confettiRadius, confettiNumber, confettiColors, emojis, emojiSize, emojiColors };
}
